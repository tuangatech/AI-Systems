...
class IntentParserAgent(BaseAgent):
    def __init__(self, model_name: str = MODEL_NAME):
        ...
        self.prompt_template = ChatPromptTemplate.from_messages([
            (
                ...
                "Rules:\n"
                "- undervalued: peRatio < 30, pbRatio < 15, freeCashFlowYield > 5\n"
                "- safe, stable: debtToEquity < 1.0, revenueGrowth > 5\n"
                "- high dividend: dividendYield > 4\n"
                "- dividend paying: dividendYield > 0\n"
                "- growth: revenueGrowth > 10, freeCashFlowYield > 5\n"
                "- under $N: price_lt = N\n"
                "- top N stocks: limit = N\n"
                "- REITs: sector = 'real_estate'\n"
                "- Default intent is 'screen'\n"
                "Example output:\n"
                '{{"intent": "screen", "sector": "technology", "limit": 3, "metrics": ["peRatio", "pbRatio"], "filters": {{"price_lt": 50, "dividendYield_gt": 0, "freeCashFlowYield_gt": 5}}}}\n'
                '{{"intent": "screen", "sector": "REIT", "limit": 5, "metrics": ["price", "dividendYield"], "filters": {{"debtToEquity_lt": 1.0, "revenueGrowth_gt": 5}}}}\n'
                "Default intent is 'screen'. Return ONLY 1 raw JSON object without any explanations or markdown formatting."
            ),
    ...
    def invoke(self, input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not sector:
            logger.info("Missing sector in query.")
            return {
                "clarification_needed": True,
        ...
        if normalized_sector not in VALID_SECTORS:
            logger.info(f"Invalid sector: {sector}")
            return {
                "clarification_needed": True,
                "error": f"'{sector}' is not a valid sector. Please try one of: {', '.join(VALID_SECTORS.keys())}",
                "parsed": parsed.model_dump()
            }
